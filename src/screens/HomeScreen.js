...
const HomeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    getPopularSeries().then((tvResponse) => setPopularSeries(tvResponse.data));

    getUpcomingSeries().then((tvResponse) => setUpcomingSeries(tvResponse.data));
  ...
  return (
    <ScrollView style={styles.container}>
      <StatusBar style="auto" translucent={false} backgroundColor={COLORS.BASIC_BACKROUND} />
      <View style={styles.headerContainer}>
        <Text style={styles.headerTitle}>Découvrir</Text>
        <Feather name="search" size={34} color={COLORS.ACTIVE} onPress={() => navigation.navigate("Search")} />
      </View>
      <View style={styles.genreListContainer}></View>
      <FlatList
