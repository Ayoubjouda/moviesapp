...
const SerieScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    getSerieById(serieId, `${AR.VIDEOS},${AR.CREDITS},${AR.RECOMMENDATIONS},${AR.SIMILAR}`).then((response) => setTv(response?.data));
  }, []);
  return (
    <ScrollView>
      ...
      <View style={styles.tvPosterImageContainer}>
        <Image style={styles.tvPosterImage} resizeMode="cover" source={{ uri: getPoster(tv?.backdrop_path) }} />
      </View>
      <View style={styles.headerContainer}>
        <TouchableOpacity activeOpacity={0.5} onPress={() => navigation.goBack()}>
          <Feather name="chevron-left" size={35} color={colors.WHITE} />
        </TouchableOpacity>
        <TouchableOpacity activeOpacity={0.5} onPress={() => Share.share({ message: `${tv?.title}\n\n${tv?.homepage}` })}>
          <Text style={styles.headerText}>Partager</Text>
        </TouchableOpacity>
      </View>
      <TouchableOpacity style={styles.playButton} onPress={() => Linking.openURL(getVideo(tv.videos.results[0].key))}>
      ...
      <Text style={styles.genreText}>
        {tv?.genres?.map((genre) => genre?.name)?.join(", ")} | {tv?.runtime} Min
      </Text>
      <Text style={styles.genreText}>{getLanguage(tv?.original_language)?.english_name}</Text>
      ...
      <View>
        ...
        <View style={styles.castSubMenuContainer}>
          <TouchableOpacity activeOpacity={0.5} onPress={() => setIsCastSelected(true)}>
            <Text style={{ ...styles.castSubMenuText, color: isCastSelected ? colors.BLACK : colors.LIGHT_GRAY }}>Acteurs</Text>
          </TouchableOpacity>
          <TouchableOpacity activeOpacity={0.5} onPress={() => setIsCastSelected(false)}>
            <Text style={{ ...styles.castSubMenuText, color: isCastSelected ? colors.LIGHT_GRAY : colors.BLACK }}>Equipe</Text>
          </TouchableOpacity>
        </View>
        <FlatList
          ...
          renderItem={({ item }) => (
            <CastCard originalName={item?.name} characterName={isCastSelected ? item?.character : item?.job} image={item?.profile_path} />
          )}
        />
      </View>
